...
class ftx(Exchange):
    ...
    def fetch_markets(self, params={}):
        ...
        for i in range(0, len(markets)):
            ...
            symbol = (
                self.safe_string(market, "name") if (type == "future") else (base + "/" + quote)
            )
    ...
    def fetch_ohlcv(self, symbol, timeframe="1m", since=None, limit=None, params={}):
        ...
        if since is None:
            ...
            request["limit"] = limit
            request["end_time"] = self.sum(
                request["start_time"], limit * self.parse_timeframe(timeframe)
            )
    ...
    def fetch_trades(self, symbol, since=None, until=None, limit=None, params={}):
        self.load_markets()
        market, marketId = self.get_market_params(symbol, "market_name", params)
        request = {
            "market_name": marketId,
        ...
        if type == "limit":
            ...
            if stopPrice is None:
                raise ArgumentsRequired(
                    self.id
                    + " createOrder() requires a stopPrice parameter or a triggerPrice parameter for "
                    + type
                    + " orders"
                )
            else:
            ...
            if price is not None:
                request["orderPrice"] = float(
                    self.price_to_precision(symbol, price)
                )  # optional, order type is limit if self is specified, otherwise market
    ...
    def edit_order(self, id, symbol, type, side, amount, price=None, params={}):
        ...
        params = self.omit(
            params,
            ["client_order_id", "clientOrderId", "triggerPrice", "orderPrice", "trailValue"],
        )
